 ...
 * music in the way. The credits sit between the two: the roll is reading, so
 * the theme steps back from full, but nothing is being timed and the music is
 * half of what a credits roll is.
 *
 * All three sit far under the cues, which must always cut through.
 */
export type MusicLevel = 'full' | 'credits' | 'bed'
...
function applyMusic(): void {
	...
	if (musicLevel === null || mutedState) {
		// Paused where it stands, never rewound, so turning sound back on picks
		// the tape up where it was rather than starting it over.
		music.volume = 0
		music.pause()
		return
	}
